...
# 
# NOTE: modified to store trained models in a custom way.
...
if __name__ == '__main__':
    ...
    parser.add_argument("-ds",  "--dataset_name", type=str, help="Dataset name")
    parser.add_argument("-npr", "--n_prototypes", type=int, help="Number of prototypes")
    parser.add_argument("-idx", "--exp_idx",      type=int, help="Experiment idx")
    parser.add_argument("-s",   "--save_dir",     type=str, help="Results directory")
    ...
    print("Experiment index:", exp_idx)
    ...
    #Use the experiment number as seed to ensure diversity
    tf.set_random_seed(1234+exp_idx)
    np.random.seed(1234+exp_idx)
    random.seed(1234+exp_idx)

    # the directory to save the model
    print("Creating saving directories...")
    makedirs(save_dir)
    img_folder = os.path.join(save_dir, "img")
    makedirs(img_folder)
    model_filename = "cae"
    ...
    console_log = open(os.path.join(save_dir, "console_log.txt"), "w+")
    ...
    save_step = 100           # how frequently do we save the model to disk
    ...
    # Network Parameters
    #n_prototypes = 15    #initialized above as parameter     # the number of prototypes
    n_layers = 4
    ...
    weights = {
        'enc_f1': tf.Variable(tf.random_normal(filter_shape_1,stddev=0.01,dtype=tf.float32), name='encoder_f1'),
        'enc_f2': tf.Variable(tf.random_normal(filter_shape_2,stddev=0.01,dtype=tf.float32), name='encoder_f2'),
        'enc_f3': tf.Variable(tf.random_normal(filter_shape_3,stddev=0.01,dtype=tf.float32), name='encoder_f3'),
        'enc_f4': tf.Variable(tf.random_normal(filter_shape_4,stddev=0.01,dtype=tf.float32), name='encoder_f4'),
        'dec_f4': tf.Variable(tf.random_normal(filter_shape_4,stddev=0.01,dtype=tf.float32), name='decoder_f4'),
        'dec_f3': tf.Variable(tf.random_normal(filter_shape_3,stddev=0.01,dtype=tf.float32), name='decoder_f3'),
        'dec_f2': tf.Variable(tf.random_normal(filter_shape_2,stddev=0.01,dtype=tf.float32), name='decoder_f2'),
        'dec_f1': tf.Variable(tf.random_normal(filter_shape_1,stddev=0.01,dtype=tf.float32), name='decoder_f1')
    }

    biases = {
        'enc_b1': tf.Variable(tf.zeros([n_map_1], dtype=tf.float32), name='encoder_b1'),
        'enc_b2': tf.Variable(tf.zeros([n_map_2], dtype=tf.float32), name='encoder_b2'),
        'enc_b3': tf.Variable(tf.zeros([n_map_3], dtype=tf.float32), name='encoder_b3'),
        'enc_b4': tf.Variable(tf.zeros([n_map_4], dtype=tf.float32), name='encoder_b4'),
        'dec_b4': tf.Variable(tf.zeros([n_map_3], dtype=tf.float32), name='decoder_b4'),
        'dec_b3': tf.Variable(tf.zeros([n_map_2], dtype=tf.float32), name='decoder_b3'),
        'dec_b2': tf.Variable(tf.zeros([n_map_1], dtype=tf.float32), name='decoder_b2'),
        'dec_b1': tf.Variable(tf.zeros([n_input_channel], dtype=tf.float32), name='decoder_b1')
    }


    last_layer = {
        'w': tf.Variable(tf.random_uniform(shape=[n_prototypes, n_classes],dtype=tf.float32), name='last_layer_w')
    }
    ...
    prototype_feature_vectors = tf.Variable(tf.random_uniform(shape=[n_prototypes, n_features],dtype=tf.float32),
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(n_train_batch):
                ...
                elastic_batch_x = batch_elastic_transform(batch_x, sigma=sigma, alpha=alpha,
            ...
            if epoch % save_step == 0 or epoch == training_epochs - 1:
                ...
                print("Model saved")
                tmp_bs, tmp_rfv = sess.run([deconv_batch_size,reshape_feature_vectors],
                                           feed_dict={feature_vectors: prototype_feature_vectors.eval()})
                prototype_imgs = sess.run(X_decoded,
                                          feed_dict={deconv_batch_size:       tmp_bs,
                                                     reshape_feature_vectors: tmp_rfv})
    ...
    with open(save_dir + "/DONE", "w") as file:
        file.write("DONE")
